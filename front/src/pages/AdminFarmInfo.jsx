  ...
  */
      ...
      try {
        ...
        if (isCreateMode) {
          ...
          if (passedFarmInfo && passedUserInfo) {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/api/farms/${farmIdx}/detail`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      if (isCreateMode) {
        ...
        const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/api/farm/insertFarm`, formData, {
          headers: {
        ...
        // 농장 수정 모드
        const response = await axios.put(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/api/admin/users/farms/${farmIdx}`, editedFarmInfo);
  ...
  const handleDeleteFarm = async () => {
    if (window.confirm(`정말로 "${farmInfo?.farmName}" 농장을 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.`)) {
      try {
        const response = await axios.delete(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/api/admin/users/farm/${farmIdx}`);
