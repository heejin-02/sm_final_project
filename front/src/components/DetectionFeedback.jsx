...
const setCachedRagResponse = (insectName, data) => {
  localStorage.setItem(RAG_CACHE_KEY(insectName), JSON.stringify({
    data,
    timestamp: Date.now()
  }));
};
...
export default function DetectionFeedback({ anlsIdx, alertDetail, onFeedbackSubmit }) {
  ...
  const getDefaultMessage = (insectName) => {
    const messages = {
      '꽃노랑총채벌레': '꽃노랑총채벌레는 작고 노란색의 해충으로 잎과 꽃을 가해합니다.',
      '담배가루이': '담배가루이는 작고 흰색의 날아다니는 해충으로 잎 뒷면에 서식합니다.',
  ...
  useEffect(() => {
    const fetchInsectInfo = async () => {
      ...
      try {
        ...
        const message = response.answer || response.message || getDefaultMessage(insectName);
        ...
        <div className="baekgu-msg">
          {isLoadingRag ? (
            <div className="loading-message">
              <span className="loading-spinner">⏳</span> 해충 정보를 분석하고 있습니다...
            </div>
          ) : (
            // GPT 결과가 있으면 우선 사용, 없으면 RAG 응답 사용
            alertDetail?.gptResult?.gptContent || ragMessage || getDefaultMessage(alertDetail?.greenhouseInfo?.insectName)
          )}
        </div>
      </div>
