...
export default function FeedbackTextarea({ anlsIdx, alertDetail }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/user/alert/feedback/${anlsIdx}`)  // 경로 변수로 요청
      .then((res) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (savedFeedback) {
        // PUT 요청으로 수정
        await axios.put(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/user/alert/feedback/${anlsIdx}`, {
        ...
        // POST 요청으로 등록
        await axios.post(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8095'}/user/alert/feedback`, {
          anlsIdx,
          feedbackContent: content,
          createdAt: new Date().toISOString(),
        });
