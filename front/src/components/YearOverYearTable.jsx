...
export default function YearOverYearTable({ stats }) {
  ...
  if (list.length === 0) {
    return (
      <div className='mt-8'>
        <div className='bordered-box p-4 text-gray-500'>
          예측 데이터가 없습니다.
        </div>
      </div>
    );
  ...
  const { insectDataMap, insectNames } = useMemo(() => {
    ...
    const base = () => ({ count2024: 0, count2025: 0, predicted2026: 0 });
