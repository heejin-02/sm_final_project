    ...
    if (error) {
        return (
            <div className={`noti-area ${isNotiOpen ? 'open' : ''}`}>
                ...
                <div className='p-4 text-center text-red-500'>알림을 불러오는데 실패했습니다.</div>
            </div>
        );
    }
        ...
        <div className={`noti-area ${isNotiOpen ? 'open' : ''}`}>
            ...
            <ul className='noti-list scrl-custom'>
                {sortedAlerts.length === 0 ? (
                    <li className='noti-item text-center text-gray-500 py-8'>알림이 없습니다.</li>
                ) : (
                    sortedAlerts.map((alert, index) => (
                        <li
                            key={alert.anlsIdx}
                            className={`noti-item ${currentNotiId === alert.anlsIdx ? 'active' : ''} ${
